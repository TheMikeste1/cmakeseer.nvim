...
---@class neotest.cmakeseer.gtest.suite.Typed: neotest.cmakeseer.gtest.suite.Basic A suite with using a known set of types. They have a postfix, but no prefix.
...
---@return neotest.cmakeseer.gtest.suite.Typed instance A new Typed instance.
function Typed:new(o)
  ---@class neotest.cmakeseer.gtest.suite.Typed
  o = Basic.new(self, o)
...
function Typed:parse_add_gtests(testsuite, files, prefix, postfix)
  ...
  for _, test in ipairs(testsuite) do
    ...
    assert(self.type_parameters[index] == test.type_param, "All tests in a suite with the same index should have the same type_param")
  end
end

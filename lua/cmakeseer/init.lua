...
function M.resolve_build_directory()
  ...
  if preset == nil then
    ...
    if configure_preset ~= nil then
      binary_dir = CMakePreset.try_determine_binary_dir(configure_preset, current_config:project_root(), CMakePreset.Types.Configure, { resolve_path = true })
    end
  else
    binary_dir = CMakePreset.try_determine_binary_dir(preset, current_config:project_root(), CMakePreset.Types.Build, { resolve_path = true })
  end
...
function M.get_build_args()
  ...
  if preset == nil then
    ...
    local binary_dir = CMakePreset.try_determine_binary_dir(preset, current_config:project_root(), CMakePreset.Types.Build)
...
function M.get_basic_configure_args()
  ...
  if preset ~= nil then
    ...
    local CMakePreset = require("cmakeseer.cmake.preset")
    binary_dir = CMakePreset.try_determine_binary_dir(preset, current_config:project_root(), CMakePreset.Types.Configure)

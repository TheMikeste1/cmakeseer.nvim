...
---@enum cmakeseer.cmake.preset.Type
local Types = {
  Configure = "configure",
  Build = "build",
  Test = "test",
...
---@param preset_type cmakeseer.cmake.preset.Type The type of preset to fetch.
...
function M.file_for(preset, dir, preset_type, opts)
  ...
  local function file_has_preset(file)
    ...
    if preset_type == Types.Configure then
      preset_list = file.configure_presets
    elseif preset_type == Types.Build then
      preset_list = file.build_presets
    elseif preset_type == Types.Test then
      preset_list = file.test_presets
    elseif preset_type == Types.Package then
      preset_list = file.package_presets
    elseif preset_type == Types.Workflow then
...
local function try_determine_value(preset, dir, preset_type, field, empty_value)
  ...
  if preset_type == Types.Workflow then
    return empty_value
  end
  ...
  if preset_type == Types.Configure then
    ...
    if value == empty_value and preset_entry.inherits ~= nil then
      ...
      for _, inheritted in ipairs(inherits) do
        value = try_determine_value(inheritted, dir, Types.Configure, field, empty_value)
    ...
    if configure_preset ~= nil then
      value = try_determine_value(configure_preset, dir, Types.Configure, field, empty_value)
    end
